...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({ pattern: 'rwolf ?(.*)', fromMe: true,dontAddCommandList: true }, (async (message, match) => {
        ...
        var ttinullimage = await axios.get(`https://api.zeks.xyz/api/watercolour?apikey=00oQhHnqcqFGofHbjsFtXZcKVWO&text1=Ini%20contoh&text2=Okey=${encodeURIComponent(match[1])}`, { responseType: 'arraybuffer' })
    ...
    Asena.addCommand({ pattern: 'rwolf ?(.*)', fromMe: false,dontAddCommandList: true }, (async (message, match) => {
        ...
        var ttinullimage = await axios.get(`https://api.zeks.xyz/api/snowwrite?apikey=00oQhHnqcqFGofHbjsFtXZcKVWO&text1=Ini%20contoh&text2=Okey=${encodeURIComponent(match[1])}`, { responseType: 'arraybuffer' })
